 ...
 *
 * Implements the provider interface for Google Gemini.
 */
class GeminiProvider extends AbstractProvider {
    ...
    private function initAgent() {
        ...
        try {
            ...
            // Set up the Gemini provider with the Neuron AI framework
            // Note: This class is provided by the Neuron AI PHP framework dependency
            $gemini_provider = new \NeuronAI\Providers\Google\Gemini(
                $this->api_key,
                $this->model
            );
            
            $agent->setProvider($gemini_provider);
            
            return $agent;
        } catch (\Exception $e) {
            throw new ProviderException(
                'Failed to initialize Gemini agent: ' . $e->getMessage(),
        ...
        try {
            ...
            if ($this->logger) {
                $this->logger->log_request('enhance', [
                    'provider' => ProviderConstants::PROVIDER_GEMINI,
        ...
        try {
            ...
            if (strpos($e->getMessage(), 'rate limit') !== false || 
                ...
                $error_code = ProviderException::ERROR_INVALID_MODEL;
                     ...
                     strpos($e->getMessage(), 'safety') !== false) {
    ...
    public function testConnection() {
        ...
        try {
            ...
            return [
                'success' => true,
                'message' => __('Successfully connected to Gemini API.', 'neuron-ai'),
                'model' => $this->model
            ];
        } catch (\Exception $e) {
            if ($this->logger) {
    ...
    public function getName() {
        return ProviderConstants::PROVIDER_GEMINI;
    }
    ...
    public function getCapabilities() {
        $model_caps = ProviderConstants::getModelCapabilities(
            ProviderConstants::PROVIDER_GEMINI,
            $this->model
        );
    ...
    public function getAvailableModels() {
        return ProviderConstants::getGeminiModels();
    }
    ...
    private function build_search_instructions($options) {
        ...
        $instructions = "You are an AI research assistant specialized in information retrieval. ";
        $instructions .= "Your task is to provide accurate, up-to-date information in response to queries. ";
        
        // Detail level instructions - optimized for Gemini's search capabilities
        switch ($detail_level) {
            case 'brief':
                $instructions .= "Provide concise summaries with only key points and essential details. Aim for clarity and brevity. ";
                break;
            case 'medium':
                $instructions .= "Provide balanced overviews with main concepts and supporting details. Include key facts and evidence. ";
                break;
            case 'comprehensive':
                $instructions .= "Provide detailed explanations with thorough analysis and multiple perspectives. Include background context, key developments, and various viewpoints. ";
                break;
            default:
                $instructions .= "Provide balanced information with appropriate level of detail. ";
        }
        
        // Citation instructions - Gemini optimized
        if ($include_citations) {
            $instructions .= "Include specific credible sources that support your information. ";
            $instructions .= "Format the citations at the end of your response with title, source, and year where applicable. ";
        }
        
        // Gemini-specific formatting instructions
        $instructions .= "Structure your response with clear HTML headings (h2, h3), organized paragraphs, and bullet points when appropriate. ";
        $instructions .= "Use a factual, informative writing style focused on accuracy and clarity. ";
    ...
    private function build_search_prompt($query, $options) {
        // Gemini-optimized search prompt
        ...
        // Gemini-specific search enhancements
        $prompt .= "Focus on accuracy, clarity, and reliable information. ";
        $prompt .= "If there are multiple interpretations of the query, briefly address the most relevant ones. ";
        $prompt .= "Organize the information in a logical structure with clear sections.";
    ...
    private function build_chat_instructions($page_context, $options) {
        $instructions = "You are a helpful content creation assistant powered by Google Gemini. ";
